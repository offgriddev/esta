...
export const getEstimateProbability = new Command()
  ...
  .action(async (data, avgSpeed, possibleHours, devCount, sampleSize) => {
    ...
    console.log(`Taking ${sampleSize} samples of a random distribution.`);
    for (let i = 0; i < sampleSize; i++) {
      const { weeks } = await estimate(
        records,
        avgSpeed,
        possibleHours,
        devCount
    ...
    console.log(`Average Estimate in Weeks: ${average}`);
    ...
    console.log(JSON.stringify(percentageOfResults, "", 2));
  });
