...
exports.estimate = exports.getSourceFile = void 0;
const promises_1 = require("fs/promises");
const lodash_groupby_1 = __importDefault(require("lodash.groupby"));
const getSourceFile = async (folder, includedType, excludedType) => {
    let filePaths = [];
    ...
    for (let p = 0; p < paths.length; p++) {
        ...
        if (path.isDirectory()) {
            if (path.name.match(/.*node_modules.*|dist.*/))
                continue;
            const recursePaths = await (0, exports.getSourceFile)(`${folder}/${path.name}`, includedType, excludedType);
...
const estimate = async (records, avgSpeed, possibleHours, numberOfDevs) => {
    ...
    for (const key of keys) {
        ...
        while (cards.length > 0) {
            ...
            // player selection
            const [player, { workable, overage }] = getPlayer(players, currentWeek, card);
            if (!player) {
